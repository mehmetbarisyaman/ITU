...
void search_record(){
	char name[AD];
	cout << "Please enter the name of the person you want to search for (press '*' for full list): ";
	cin.ignore(1000, '\n');
	cin.getline(name, AD);
	if (book.search(name) == 0){
		cout << "Could not find a record " << endl;
	}
	getchar();
};
void add_record(){
	...
	cout << "Name : ";
	cin.ignore(1000, '\n');
	cin.getline(newrecord.name, AD);
	cout << "Phone number :";
	cin >> setw(NO) >> newrecord.phonenum;
	book.add(&newrecord);
...
void delete_record(){
	char name[AD];
	int choice;
	cout << "Please enter the name of the person whose record you want to delete (press '* 'for full  list): ";
	cin.ignore(1000, '\n');
	cin.getline(name, AD);
	...
	if (personcount == 0){
		...
		if (personcount == 1){
			...
			cout << "If you want to delete this record, please enter its record number (Enter -1 to exit without performing any operations): ";
		}
		else
