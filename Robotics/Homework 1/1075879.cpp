...
void laser_callback(const sensor_msgs::LaserScan::ConstPtr& msg)
{
    ...
    float frontDistance= laser_ranges[laser_ranges.size()/2];
    float leftDistance= laser_ranges[laser_ranges.size()-1];
    float rightDistance= laser_ranges[0];
    
    /* 
    Since the max angle that robot perceive is approximately 0.5,
     The angle that robot turns should be 0.5
     */
    
    if(frontDistance < 1.2 && leftDistance<1.2 && rightDistance<1.2){
		// Robot turns backwards
		motor_command.angular.z = 8.0;
		motor_command.linear.x = 0;
	}
	else if(frontDistance < 1.2 && leftDistance<1.2){
		// Robot turns right
		motor_command.angular.z = -0.5;
		motor_command.linear.x = 0;
	}
	else if(frontDistance < 1.2 && rightDistance<1.2){
		// Robot turns left
		motor_command.angular.z = 0.5;
		motor_command.linear.x = 0;
	}
	else if(leftDistance<1.2 && rightDistance<1.2){
		// This is important when robot passes any door. Going straight is needed
    ...
    else if(leftDistance<1.2){
		// Robot turns right
		motor_command.angular.z = -0.5;
		motor_command.linear.x = 0;
	}
	else if(rightDistance<1.2){
		// Robot turns left
		motor_command.angular.z = 0.5;
		motor_command.linear.x = 0;
	}
